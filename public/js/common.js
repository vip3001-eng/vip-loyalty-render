...
function ensureSecureContext(){
  // الكاميرا تحتاج سياق آمن: HTTPS أو localhost.
  // ملاحظة مهمة: المتصفحات تعتبر http://localhost سياقًا آمنًا — لذلك لا نجبر التحويل.
  const isLocal = location.hostname === 'localhost' || location.hostname === '127.0.0.1';
  if(isLocal) return true;
  if(location.protocol !== 'https:'){
    // للمستخدم النهائي: نفضّل التحويل التلقائي للنسخة الآمنة حتى تعمل الكاميرا.
    // إذا كانت الشهادة غير موثوقة سيظهر تحذير متصفح ويمكن للمستخدم متابعة.
...
// رابط النسخة الآمنة (للتجربة على الجوال عبر IP)
// ملاحظة: السيرفر يشغّل HTTPS محليًا على 3443 (انظر server.js)
...
// نفس showNotice لكن يدعم HTML (لا تستخدمه مع مدخلات المستخدم)
...
function closeModal(){
  ...
  // صفحات العملاء: عند إغلاق نافذة الباركود يرجع تلقائياً للرئيسية
...
// ماسح QR / باركود
// الأفضل: Html5Qrcode (يدعم أغلب المتصفحات) — نحمّله من /vendor/html5-qrcode.min.js
// بديل: BarcodeDetector (إذا كان مدعومًا)
    ...
    if(window.Html5Qrcode){
      try{
        ...
        await h5.start(
          ...
          (_errMsg)=>{ /* تجاهل أخطاء القراءة المتكررة */ }
        );

        return;
      }catch(e){
        // إذا فشل Html5Qrcode لأي سبب، نجرب البديل BarcodeDetector (إذا متوفر)
        try{
          if(h5){
        ...
        // لا نرمي هنا مباشرة — نكمل للبديل
      }
    }

    // 2) بديل BarcodeDetector
...
async function api(path, method="GET", body=null){
  ...
  const data = await res.json().catch(()=>({ ok:false, error:"BAD_JSON", message:"استجابة غير صالحة من الخادم" }));
...
// دخول الأدمن مخفي داخل الشعار: اضغط عدة مرات بسرعة
...
// صوت بسيط (مثل جهاز الكاشير) عند قراءة QR
function playBeep(){
  try{
...
// أيقونات (SVG) بدون أي مكتبات خارجية
